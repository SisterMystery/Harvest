...
class controlPoint(blobPoint):
	def __init__(self,x,y,minions):
		...
		self.parts = minions
		self.controls = []
		self.image = pygame.image.load("images/control.bmp")
	...
	def accelerate(self):
		#	self.position = self.position.add(self.velocity)
			...
			#		point.velocity = physics.vector2d(0,0)
				...
				#if dist <= 50:
				#	point.velocity = point.velocity.multiply(.8)
				"""
				if dist >= 200:
			...
			for point in self.parts:#.controls:
				...
				if dist > 20 and newdist > 20  and point.kind == "blobPoint" : #and newdist >  50:
				...
				if dist < 2 and point.kind == "blobPoint" :#or newdist < 40:
					#point.velocity = vect.unitize().multiply(-10)
					physics.applyForce(point,vect.reverse().multiply(5))
